...
public interface ProjectRoleMapper extends BaseMapper<ProjectRole> {
    ...
    List<ProjectRole> findByProjectId(@Param("projectId") Long projectId);
    ...
    ProjectRole findByProjectAndUser(@Param("projectId") Long projectId, @Param("userId") Long userId);

    @Select("SELECT * FROM project_role WHERE project_id = #{projectId} AND role = #{role} AND deleted = 0")
    List<ProjectRole> findByProjectAndRole(@Param("projectId") Long projectId, @Param("role") String role);

    @Delete("DELETE FROM project_role WHERE project_id = #{projectId} AND user_id = #{userId}")
    void deleteByProjectAndUser(@Param("projectId") Long projectId, @Param("userId") Long userId);
}
